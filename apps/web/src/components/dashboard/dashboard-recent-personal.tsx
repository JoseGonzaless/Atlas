...
export function DashboardRecentPersonal({ expenses }: Props) {
  return (
    <div className="rounded-xl border bg-card p-4 flex flex-col">
      <div className="flex items-start justify-between gap-2 pb-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-muted-foreground">Permanent budget history</p>
        </div>
        <Link
          to="/personal"
      ...
      {expenses.length === 0 ? (
        <p className="text-sm text-muted-foreground py-2">No personal expenses yet.</p>
      ) : (
              ...
              <div key={e.id} className="flex items-center gap-3 px-4 py-2.5">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5">
                    {format(e.date, "MMM d '·' h:mma")} · {e.category}
                  </p>
                </div>
